...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Program make_initial_model_dix_ex1 is a Matlab script to make an 
% initial model for use in iterative Love wave phase velocity 
% inversion using the Dix method.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% Power law fit
[aap bbp] = polyfit(log(xcof*c./fr),log(c/xiaf),1);
% Linear fit
[aal bbl] = polyfit(xcof*c./fr,c/xiaf,1);

% extrapolation to surface
if (c(end)/xiaf - exp(aap(2))*aap(1)*((xcof*c(end)/fr(end))^aap(1)) > 0)
    czro = c(end)/xiaf - exp(aap(2))*aap(1)*((xcof*c(end)/fr(end))^aap(1));
    xiatrap = [c/xiaf czro];
elseif (c(end)/xiaf - aal(1)*xcof*c(end)/fr(end) > 0)
    czro = c(end)/xiaf - aal(1)*xcof*c(end)/fr(end);
    xiatrap = [c/xiaf czro];
else
    czro = c(end)/xiaf;
    xiatrap = [c/xiaf czro];
...
if (c(1)/xiaf - exp(aap(2))*aap(1)*((xcof*c(1)/fr(1))^(aap(1)-1))*(xcof*c(1)/fr(1)-max(z_mid)) > 0)
    cdeep = c(1)/xiaf - exp(aap(2))*aap(1)*((xcof*c(1)/fr(1))^(aap(1)-1))*(xcof*c(1)/fr(1)-max(z_mid));
    xiatrap = [cdeep xiatrap];
elseif (c(1)/xiaf - aal(1)*(xcof*c(1)/fr(1)-max(z_mid)) > 0)
    cdeep = c(1)/xiaf - aal(1)*(xcof*c(1)/fr(1)-max(z_mid));
    xiatrap = [cdeep xiatrap];
else
    cdeep = c(1)/xiaf;
    xiatrap = [cdeep xiatrap];
...
fprintf(fidt,'%% input parameters for Love wave inversion\n');
fprintf(fidt,'\n');
