...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Program make_initial_model_dix_ex2 is a Matlab script to make an initial 
% model from Rayleigh dispersion using the Dix method for use in iterative 
% Love wave phase velocity inversion.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
c = load('velocity_values.txt');
% frequency raster, a row vector from low to high frequency
fr = load('frequency_values.txt');
% use only the fundamental mode
c = c(1:81)';
fr = fr(1:81)';
...
% a smoothed version of velocities used for extrapolation
spts = 10;
cxs = convsm_1d(c,spts); % smooth over nearest (2spts + 1) velocities
...
% Power law fit
[aap bbp] = polyfit(log(xcof*cxs./fr),log(cxs/0.88),1);
% Linear fit
[aal bbl] = polyfit(xcof*cxs./fr,cxs/0.88,1);

% extrapolation to surface
if (cxs(end)/.88 - exp(aap(2))*aap(1)*((xcof*cxs(end)/fr(end))^aap(1)) > 0)
    czro = cxs(end)/.88 - exp(aap(2))*aap(1)*((xcof*cxs(end)/fr(end))^aap(1));
    xiatrap = [cxs/0.88 czro];
elseif (cxs(end)/.88 - aal(1)*xcof*cxs(end)/fr(end) > 0)
    czro = cxs(end)/.88 - aal(1)*xcof*cxs(end)/fr(end);
    xiatrap = [cxs/0.88 czro];
else
    czro = cxs(end)/.88;
    xiatrap = [cxs/0.88 czro];
...
if (cxs(1)/.88 - exp(aap(2))*aap(1)*((xcof*cxs(1)/fr(1))^(aap(1)-1))*(xcof*cxs(1)/fr(1)-max(z_mid)) > 0)
    cdeep = cxs(1)/.88 - exp(aap(2))*aap(1)*((xcof*cxs(1)/fr(1))^(aap(1)-1))*(xcof*cxs(1)/fr(1)-max(z_mid));
    xiatrap = [cdeep xiatrap];
elseif (cxs(1)/.88 - aal(1)*(xcof*cxs(1)/fr(1)-max(z_mid)) > 0)
    cdeep = cxs(1)/.88 - aal(1)*(xcof*cxs(1)/fr(1)-max(z_mid));
    xiatrap = [cdeep xiatrap];
else
    cdeep = cxs(1)/.88;
    xiatrap = [cdeep xiatrap];
...
xia_int = interp1([max(z_mid) xcof*cxs./fr 0],xiatrap,z_mid,'linear');
...
ylabel('Model correlation length factor','FontSize',fsize,...
    'FontWeight','bold')
orient landscape
print(gcf,'-dpsc','-r300','-opengl','chi_squared.ps');
